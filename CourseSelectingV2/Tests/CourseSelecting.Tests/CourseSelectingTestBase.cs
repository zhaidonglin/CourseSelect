...
namespace CourseSelecting.Tests
{
    public abstract class CourseSelectingTestBase : AbpIntegratedTestBase<CourseSelectingTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected CourseSelectingTestBase()
        {
        ...
        protected void UsingDbContext(Action<CourseSelectingDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Action<CourseSelectingDbContext> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<CourseSelectingDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<CourseSelectingDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<CourseSelectingDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CourseSelectingDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Action<CourseSelectingDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<CourseSelectingDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<CourseSelectingDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<CourseSelectingDbContext, Task<T>> func)
        {
            T result;

...
using CourseSelecting.Web.Controllers.Results;
using CourseSelecting.Web.Models.Account;
...
{
    public class AccountController : CourseSelectingControllerBase
    {
        ...
        {
            var geetest = new GeetestLib(Const.PublicKey, Const.PrivateKey);
            var userCheckCode = new Guid().ToString();
        ...
        [UnitOfWork]
        [DisableAuditing]
        public virtual async Task<ActionResult> StudentRegister(StudentRegisterViewModel model)
        {
            try
            {
                CheckModelState();

                var user = await _userRepository.FirstOrDefaultAsync(x => x.UserName == model.LoginId);
                if (user != null) throw new UserFriendlyException($"学号{model.LoginId}重复");

                #region 根据学号处理入学时间年级
                if (model.LoginId.Length < 4) throw new UserFriendlyException("学号格式不正确");
                var yearstr = model.LoginId.Substring(0, 4);
                int year;
                try
                {
                ...
                var student = new Student
                {
                    ...
                    Name = model.Name,
                    Surname = model.Name,
                    EmailAddress = Users.User.SystemEmail,
                    IsActive = true,
                    Password = new PasswordHasher().HashPassword(model.Pwd),
                    StudentNo = model.LoginId,
                    Tel = model.Tel,
                    Gender = model.Gender,
                    EntryDate = entryDate,
                    Grade = gradestr
                ...
                student.Roles = new List<UserRole> { new UserRole { RoleId = role.Id } };

                await _studentRepository.InsertAsync(student);
                await _unitOfWorkManager.Current.SaveChangesAsync();

                var loginResult = await GetLoginResultAsync(student.UserName, model.Pwd, null);
        ...
        [UnitOfWork]
        [DisableAuditing]
        public virtual async Task<ActionResult> TeacherRegister(TeacherRegisterViewModel model)
        {
            try
            {
                CheckModelState();
                ...
                var user = await _userRepository.FirstOrDefaultAsync(x => x.UserName == model.LoginId);
                if (user != null) throw new UserFriendlyException($"用户名{model.LoginId}重复");
                ...
                teacher.Roles = new List<UserRole> { new UserRole { RoleId = role.Id } };

                await _teacherRepository.InsertAsync(teacher);
                await _unitOfWorkManager.Current.SaveChangesAsync();

                var loginResult = await GetLoginResultAsync(teacher.UserName, model.Pwd, null);
        ...
        public async Task<JsonResult> Login(LoginViewModel loginModel, string returnUrl = "", string returnUrlHash = "", string challenge = "", string validate = "", string seccode = "")
        {
            try
            {
                ...
                if (result != 1) throw new UserFriendlyException("验证码错误！");

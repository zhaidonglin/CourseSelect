(function () {
    $(function () {
        var examTimeService = abp.services.app.examTimeV2;
                        ...
                        success: {
                            ...
                            callback: function () {
                                ...
                                if ($("#formAddCourseTime").data("bootstrapValidator").isValid()) {
                                    ...
                                    console.log(date.format("yyyy-MM-dd ") + $("#courseTime").val());
                                    console.log($("#courseTime").val());
                                    examTimeService.addAndGetObj({
                                        TeacherCourseId: originalEventObject.id,
                                        Start: date.format("yyyy-MM-dd ") + $("#courseTime").val(),
                                        End: $("#examEndTime").val(),
                                        Address: $("#address").val(),
                                        FitGrade: checkboxValue
                                    }).done(function (result) {
                                        console.log(result);
                ...
                bootbox.dialog({
                    ...
                    buttons: {
                        success: {
                            ...
                            className: "btn-blue",
                            callback: function () {
                                examTimeService.delete(calEvent.id).done(function () {
...
function BootboxContent() {
    ...
    html += '               <div class="input-group">';
    html += '                   <input class="form-control endTime" id="examEndTime" type="text" name="examEndTime" data-date-format="yyyy-mm-dd">';
    html += '                   <span class="input-group-addon">';
    ...
    html += '           </div>';
    html += '           <div class="form-group">';
    html += '               <label>考试地点</label>';
    html += '               <input type="text" class="form-control" placeholder="考试地点" id="address" name="address">';
    html += '           </div>';
    html += '           <div class="form-group">';
    ...
    object.find('.endTime').datepicker({ language: 'zh-CN', autoclose: true });
...
function BootboxDetailsContent(id, courseName, startTime, endTime, address, limits, times, enabledList) {
    ...
    html += '                       <a class="btn btn-default" href="/Teacher/GetExamTimeOrderdStudents/' + id + '">详细信息</a>';
    html += '                   </span>';
    html += '               </div>';
    html += '           </div>';
    html += '           <div class="form-group">';

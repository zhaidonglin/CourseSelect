    ...
    $(function () {
        ...
        $('#subjectdatatable').dataTable({
            ...
            ajax: function (data, callback, settings) {
                ...
                subjectService.getStudentSelectCourses(input).done(function (result) {
                    var callbackObj = {
                        darw: data.darw,
...
function fnSelectSubject(id) {
    ...
    abp.message.confirm("您确定要选择本次课程么？", function (isconfirm) {
        if (isconfirm) {
            ...
            subjectService.addSelectSubjectProject1({
                //studentId:9,
                studentId: $("#lblStudentId").val(),
                courseId: id
...
function fnDeleteSubject(id) {
    ...
    abp.message.confirm("您确定要取消本次课程么？", function (isconfirm) {
        if (isconfirm) {
                         //selectDeleteSubjectProject
            subjectService.selectDeleteStudentCourse($("#lblStudentId").val(),id).done(function () {
